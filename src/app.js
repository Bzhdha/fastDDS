(function () {
  ...
  $("btn-grand-texte").addEventListener("click", function () {
    const on = document.documentElement.classList.toggle("grand-texte");
    this.setAttribute("aria-pressed", on);
  });
  $("btn-contraste").addEventListener("click", function () {
    const on = document.documentElement.classList.toggle("contraste-eleve");
    this.setAttribute("aria-pressed", on);
  });
  ...
  function updateMapMarkers(permanents, collectes) {
    ...
    function ajouterMarqueur(item, mapId, isPerm) {
      if (!item.latitude || !item.longitude) return;
      if (count >= MAX_MARQUEURS) return;
      count++;

      const nom     = item.name || item.address1 || item.convocationLabel || "";
      const adresse = [item.address1, item.city].filter(Boolean).join(", ");
    ...
    permanents.forEach((item, i) => ajouterMarqueur(item, `p-${i}`, true));
    collectes.forEach((item, i)  => ajouterMarqueur(item, `c-${i}`, false));

    if (bounds.length) map.fitBounds(bounds, { padding: [48, 48], maxZoom: 14 });

    $("carte-desc").textContent = count
      ? `${count} marqueur${count > 1 ? "s" : ""} affiché${count > 1 ? "s" : ""} sur la carte.`
